...
const Sidebar: React.FC<SidebarProps> = ({ storeId }) => {
  ...
  return (
    <div className="w-64 bg-gray-800 text-white h-full p-4 space-y-6 flex flex-col fixed inset-y-0 left-0"> {/* Added fixed positioning and flex-col, space-y-6 */}
      <Link href={`/${storeId}`} passHref>
        <h2 className="text-2xl font-semibold text-center hover:text-gray-300 transition-colors cursor-pointer py-2"> {/* Added padding and hover effect */}
          Dashboard
        </h2>
      </Link>
      <nav className="flex-grow"> {/* Added flex-grow to push items down if footer existed */}
        <ul className="space-y-2"> {/* Added space-y-2 for list items */}
          {routes.map((route) => (
            <li key={route.href}>
              <Link href={route.href} passHref>
                <a className={cn(
                  ...
                  route.active ? "bg-primary text-primary-foreground shadow-md" : "text-gray-300 hover:text-white" // Active link styling
                )}>
                  {route.icon}
                  <span className="ml-3">{route.label}</span> {/* Ensured margin for icon even if label is short */}
                </a>
              </Link>
            </li>
          ))}
